...
jest.mock('next/dynamic', () => () => {
  return ({ children }) => children; // Render children directly
});

// Mock react-owl-carousel3
jest.mock('react-owl-carousel3', () => {
  return ({ children, className }) => (
    <div className={className} data-testid="mock-carousel">
      {children}
    </div>
  );
});
...
describe('PartnerStyleOne Component', () => {
  ...
  test.skip('renders OwlCarousel after component mounts', async () => {
    await act(async () => {
      render(<PartnerStyleOne />);
    });
